...
_MANIFEST_FILENAME = ".shell-gen-manifest"
...
def _read_manifest(directory: Path) -> List[str]:
    """Read the list of previously generated filenames from the manifest."""
...
def _write_manifest(directory: Path, filenames: List[str]):
    """Write a manifest file listing the generated filenames."""
    lines = ["# Generated by generate_shell.py -- DO NOT EDIT"]
    lines.extend(sorted(filenames))
...
def generate_all(manifest: Dict, target: Optional[Path],
    ...
    # In target mode, read previous manifests so we can remove stale files
    prev_dir_files: Dict[Path, List[str]] = {}
    if target is not None:
        for shell_dirs in dirs.values():
            for d in shell_dirs.values():
                prev_dir_files[d] = _read_manifest(d)

    generated_files: List[Path] = []
    # Track filenames per directory for .gitignore / manifest
    ...
    if target is None:
        # Write per-directory .gitignore files (source dir only)
        for d, names in dir_files.items():
            if names:
                _write_gitignore(d, names)
    else:
        # In target mode: remove stale files and write manifests
        for d, names in dir_files.items():
            new_set = set(names)
            for old_name in prev_dir_files.get(d, []):
